...
- 카드 id는 프로젝트 최상위 기준 폴더 경로다 (예: `backend/booking`). 명령에 쓰는 경로도 모두 이 기준이다.
...
| priority | 같은 부모 아래 형제 중 중요도. 1이 가장 중요 | |
...
- **중요도**: priority가 작을수록 더 중요하다. 같은 부모 아래 형제끼리만 비교한다. 무엇부터 할지는 정하지 않는다.
- **작업 순서**: A needs B면 B를 먼저 한다. `garden map --order`가 단계별로 보여 준다.
...
- 파일을 열면 그 폴더의 `[garden]` 블록이 주입된다.
  ...
  - `siblings`: 형제 폴더의 중요도 순서와 이 폴더의 위치

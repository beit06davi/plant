...
allowed-tools:
  ...
  - Edit
---
garden 명령은 Bash 도구에서 `garden <명령>`으로 실행한다. 형식이 처음이면 `/garden:guide`를 먼저 읽는다.
...
2. SEED.md에 채우지 않은 틀(`<…>`)이 있으면 사용자에게 한 번에 한두 개씩 묻는다.
   - 이 프로젝트가 왜 있나
...
4. 상위 폴더부터 `garden add`로 쓴다. 경로는 프로젝트 최상위 기준이고, needs 대상 카드가 먼저 있어야 한다.
5. `garden check`에 오류가 없을 때까지 고친다(종료 코드 2가 아니면 된다). 경고는 이유가 있으면 남겨도 된다.
